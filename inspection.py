...
def format_tuning(dataset):
    try:
        if dataset.features["source"].feature._type == "Value": # One row of source is a line in article.
...
def stats_cal(dataset, dataset_name: str, tokenization_method: str = "whitespace", stats_to_compute: List[str] = ["SampleNum", "mean", "median", "std", "compression_ratio"]) -> NamedTuple:
    ...
    if dataset.features["source"]._type == "Value": # One row of source is one article.
        if tokenization_method == "whitespace":
            dataset = dataset.to_pandas()
    elif dataset.features["source"]._type == "Sequence": # One row of source is a line in article or combined with article, summary and id.
    ...
    if "SampleNum" in stats_to_compute:
        print(f"[{dataset_name}] Number of samples of article or summary: {stats.src.SampleNum}")
    if "mean" in stats_to_compute:
        stats.src.mean = stats_src.mean
        stats.tg.mean = stats_tg.mean
        print(f"[{dataset_name}] Mean of article & summary: {stats.src.mean:.2f}, {stats.tg.mean:.2f}")
    if "median" in stats_to_compute:
        stats.src.median = stats_src.median
        stats.tg.median = stats_tg.median
        print(f"[{dataset_name}] Median of article & summary: {stats.src.median:.2f}, {stats.tg.median:.2f}")
    if "std" in stats_to_compute:
        stats.src.std = stats_src.std
        stats.tg.std = stats_tg.std
        print(f"[{dataset_name}] Standard Deviation of article & summary: {stats.src.std:.2f}, {stats.tg.std:.2f}")
    if "compression_ratio" in stats_to_compute:
        ...
        print(f"[{dataset_name}] ratio of article/summary: {stats.src.compression_ratio:.2f}")

    # print(len(stats_tg.lens[np.where(stats_tg.lens == 0)]))
    return stats

def print_stats(dataset, dataset_name: str, tokenization_method: str = "whitespace") -> None:
...
nlp = en_core_web_sm.load(disable=("tok2vec", "tagger", "lemmatizer", "ner")) # Disabling components for only tokenization use.
# load cnn_dailymail
